        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="px-6 py-6 lg:px-8">
                ...
                <form class="space-y-6" action="#">
                    <div>
                        <label for="title" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Code Block Title</label>
                    ...
                    <div>
                        ...
                        <CodeEditor v-model="sendData.code" font-size="15px" width="100%" :header="true" :languages="[['python', 'Python'], ['cpp', 'c++'], ['html', 'HTML'], ['js','JavaScript']]" :line-nums="true"/>
                    </div>
...
export default {
    ...
    methods: {
        async getResponse() {
            const response = await axios.get('http://codebranch.me/api/getBlocks', {headers: this.headers, withCredentials: true})
            this.answer = response.data["strings"]
            },
        
        async sendBlock() {
            const response = await axios.post('http://codebranch.me/api/addBlock', this.sendData, {headers: this.headers, withCredentials: true})
